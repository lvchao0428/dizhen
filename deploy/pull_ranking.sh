...
mkdir -p "$LOCAL_ROOT/monitor/ranking_output/$EVENT_ID"
mkdir -p "$LOCAL_ROOT/monitor/data/sequences"

echo "==> $REMOTE:$REMOTE_ROOT -> $LOCAL_ROOT"
rsync -avz \
  "$REMOTE:$REMOTE_ROOT/monitor/ranking_output/$EVENT_ID/" \
  "$LOCAL_ROOT/monitor/ranking_output/$EVENT_ID/"
rsync -avz \
  "$REMOTE:$REMOTE_ROOT/monitor/data/sequences/${EVENT_ID}_eq.csv" \
  "$LOCAL_ROOT/monitor/data/sequences/" 2>/dev/null || true

echo "==> 本地文件:"
ls -la "$LOCAL_ROOT/monitor/ranking_output/$EVENT_ID/"
echo ""
