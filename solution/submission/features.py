...
def estimate_b_value(mags: np.ndarray, mc: float = MC_DEFAULT) -> float:
    ...
    if mask.sum() < 3:
        return 1.0
    log_n = np.log10(cum[mask])
    m_vals = m_bins[mask]
    slope, _ = np.polyfit(m_vals, log_n, 1)
    b = -slope * math.log(10)
...
def build_features(
    ...
    """
    构建无泄漏特征向量及统计基线（用于 stacking）。
    cutoff 前余震可用于特征；标签来自目标窗口（外部提供）。
    """
    cutoff = WINDOW_CUTOFF[window]
    t_start, t_end = WINDOW_BOUNDS[window]

    depth = float(mainshock_depth) if not (mainshock_depth is None or (isinstance(mainshock_depth, float) and np.isnan(mainshock_depth))) else 20.0
